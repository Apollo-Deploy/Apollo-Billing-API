...
class EnforceServiceTest {
    ...
    @Test
    fun `unknown app slug returns Rejected 422`() =
        runBlocking {
    ...
    @Test
    fun `NoSubscription error returns Rejected 404`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.enforceFeature(any(), any()) } returns
                Either.Left(
    ...
    @Test
    fun `QuotaExceeded returns Rejected 402 with resource, current, limit fields`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.enforceQuota(any(), any(), any()) } returns
                Either.Left(
    ...
    @Test
    fun `FeatureNotAvailable returns Rejected 402 with feature and currentPlan fields`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.enforceFeature(any(), any()) } returns
                Either.Left(
    ...
    @Test
    fun `MeterExhausted returns Rejected 402`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.enforceMeter(any(), any(), any()) } returns
                Either.Left(
    ...
    @Test
    fun `ServiceUnavailable returns Rejected 503`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.enforceFeature(any(), any()) } returns
                Either.Left(
    ...
    @Test
    fun `successful check returns Allowed`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
    ...
    @Test
    fun `BillingCheck_Quota delegates resource and limitKey to enforceQuota`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
    ...
    @Test
    fun `BillingCheck_Feature delegates feature to enforceFeature`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.enforceFeature(any(), any()) } returns Either.Right(Unit)
    ...
    @Test
    fun `BillingCheck_Meter delegates meterKey and needed to enforceMeter`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
    ...
    @Test
    fun `property - BillingError-to-result mapping is total (Property 6)`() =
        runBlocking {
            val errors: List<BillingError> =
                listOf(
