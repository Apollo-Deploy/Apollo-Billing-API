...
class InformationLeakageHttpTest {
    ...
    private val dangerousStrings = listOf(
        "Exception", "stackTrace", ".kt:", "at com.", "NullPointer",
        "jdbc:", "postgresql://", "HikariPool", "Connection refused",
        "POLAR_API_KEY", "POLAR_WEBHOOK_SECRET", "PLATFORM_CLIENT_SECRET",
        "billing_app", "billing_superuser", "billing_subscriptions",
        "/auth/jwks", "EdDSA", "Ed25519",
        "/Users/", "/home/", "apollo-billing-api",
    )
    ...
    @Test
    fun `enforce 422 does not leak internal details`() = billingTestApplication(
    ...
    @Test
    fun `enforce 503 does not leak DB connection details`() = billingTestApplication(
        routes = { noAuthInternalRoutes { enforceRoutes(enforceController) } },
    ) {
        coEvery { enforceService.enforce(any(), any()) } returns EnforceResult.Rejected(
            statusCode = 503,
    ...
    @Test
    fun `enforce malformed body does not leak serializer internals`() = billingTestApplication(
        routes = { noAuthInternalRoutes { enforceRoutes(enforceController) } },
    ) {
        val r = client.post("/internal/billing/enforce") {
            header(HttpHeaders.Authorization, "Bearer ${validServiceToken()}")
    ...
    @Test
    fun `customer billing Polar 422 does not expose raw Polar error`() = billingTestApplication(
        ...
        val r = client.patch("/internal/billing/customer/billing-info") {
            header(HttpHeaders.Authorization, "Bearer ${validServiceToken()}")
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `customer billing Polar 502 does not expose upstream infrastructure`() = billingTestApplication(
        routes = { noAuthInternalRoutes { customerBillingRoutes(customerController) } },
    ) {
        coEvery { customerService.updateBillingInfo(any()) } returns
            CustomerBillingResult.PolarFailure(
    ...
    @Test
    fun `customer billing Polar 500 does not expose Polar stack trace`() = billingTestApplication(
        routes = { noAuthInternalRoutes { customerBillingRoutes(customerController) } },
    ) {
        coEvery { customerService.updateBillingInfo(any()) } returns
            CustomerBillingResult.PolarFailure(
    ...
    @Test
    fun `webhook 401 does not reveal HMAC implementation`() = billingTestApplication(
        ...
        val r = client.post("/webhooks/polar") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `webhook 500 does not reveal handler implementation`() = billingTestApplication(
        routes = { polarWebhookRoutes(webhookController) },
    ) {
    ...
    @Test
    fun `auth failure does not reveal which field failed (iss vs aud vs exp)`() = billingTestApplication(
        routes = { noAuthInternalRoutes { enforceRoutes(enforceController) } },
    ) {
        val r = client.post("/internal/billing/enforce") {
