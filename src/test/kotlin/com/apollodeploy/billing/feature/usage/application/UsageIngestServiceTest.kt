...
class UsageIngestServiceTest {
    ...
    @Test
    fun `inbound usage is rejected before persistence when inbound receiving is not entitled`() =
        runBlocking {
            val deniedService =
                UsageIngestService(
                    repo,
    ...
    @Test
    fun `entitled inbound usage uses the message id as its idempotency key`() =
        runBlocking {
            val inboundService =
                UsageIngestService(
                    repo,
