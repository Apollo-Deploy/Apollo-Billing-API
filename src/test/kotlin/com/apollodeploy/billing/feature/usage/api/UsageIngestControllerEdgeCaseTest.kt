...
class UsageIngestControllerEdgeCaseTest {
    ...
    @Test
    fun `POST usage ingest with quantity 0 returns HTTP 200 or 202`() = billingTestApplication(
    ...
    @Test
    fun `POST usage ingest with blank orgId is not rejected at HTTP boundary`() = billingTestApplication(
        routes = { noAuthInternalRoutes { usageIngestRoutes(controller) } },
    ) {
        coEvery { usageIngestService.ingest(any()) } returns UsageIngestResponse(accepted = true, reason = null)
    ...
    @Test
    fun `POST usage ingest missing eventKey returns HTTP 400`() = billingTestApplication(
        routes = { noAuthInternalRoutes { usageIngestRoutes(controller) } },
    ) {
        val response = client.post("/internal/billing/usage/ingest") {
            header(HttpHeaders.Authorization, "Bearer ${validServiceToken()}")
    ...
    @Test
    fun `POST usage ingest missing orgId returns HTTP 400`() = billingTestApplication(
        routes = { noAuthInternalRoutes { usageIngestRoutes(controller) } },
    ) {
        val response = client.post("/internal/billing/usage/ingest") {
            header(HttpHeaders.Authorization, "Bearer ${validServiceToken()}")
