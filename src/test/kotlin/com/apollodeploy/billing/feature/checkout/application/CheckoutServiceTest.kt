...
class CheckoutServiceTest {
    ...
    @Test
    fun `unknown product returns UnknownProduct with matching appSlug and productSlug`() = runTest {
    ...
    @Test
    fun `product found but null checkout session returns Unavailable`() = runTest {
        every { repo.findProduct(any(), any()) } returns BillingProduct(
            appSlug = "signal",
            slug = "signal-ignite",
    ...
    @Test
    fun `product found and session created returns Created with id, url, and productKind`() = runTest {
        every { repo.findProduct(any(), any()) } returns BillingProduct(
            appSlug = "signal",
            slug = "signal-ignite",
            polarProductId = "polar_123",
    ...
    @Test
    fun `system metadata keys override caller-supplied values and extra caller keys are preserved`() = runTest {
        every { repo.findProduct(any(), any()) } returns BillingProduct(
            appSlug = "signal",
            slug = "signal-ignite",
            polarProductId = "polar_123",
