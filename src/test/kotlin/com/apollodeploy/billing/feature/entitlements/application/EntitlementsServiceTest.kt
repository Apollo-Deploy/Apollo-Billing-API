...
class EntitlementsServiceTest {
    ...
    @Test
    fun `null enforcer returns UnknownApp with matching appSlug`() =
        runBlocking {
    ...
    @Test
    fun `SubscriptionNotFoundError returns NoSubscription`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.resolveEntitlements(any()) } returns
                arrow.core.Either.Left(
                    com.apollodeploy.billing.core.BillingError
                        .NoSubscription("org_1", "signal"),
                )
    ...
    @Test
    fun `unexpected exception returns InternalError`() =
        runBlocking {
            every { repo.getEnforcer("signal") } returns enforcer
            coEvery { enforcer.resolveEntitlements(any()) } returns
                arrow.core.Either.Left(
                    com.apollodeploy.billing.core.BillingError
                        .ServiceUnavailable("signal-db", "db down"),
                )
    ...
    @Test
    fun `happy path returns Found with all response fields populated`() =
        runBlocking {
            val limits =
                PlanFeatureConfig(
    ...
    @Test
    fun `remaining values with UNLIMITED_SENTINEL are mapped to toString()`() =
        runBlocking {
            val limits =
                PlanFeatureConfig(
