...
class EntitlementsControllerTest {
    ...
    @Test
    fun `GET entitlements without Authorization header returns HTTP 401`() = billingTestApplication(
    ...
    @Test
    fun `GET entitlements Found returns HTTP 200 with all fields`() = billingTestApplication(
        routes = { noAuthInternalRoutes { entitlementsRoutes(controller) } },
    ) {
    ...
    @Test
    fun `GET entitlements UnknownApp returns HTTP 404 with message field`() = billingTestApplication(
        routes = { noAuthInternalRoutes { entitlementsRoutes(controller) } },
    ) {
        coEvery { entitlementsService.getEntitlements("signal", "org_1") } returns
    ...
    @Test
    fun `GET entitlements NoSubscription returns HTTP 404 with billing_no_subscription code`() = billingTestApplication(
        routes = { noAuthInternalRoutes { entitlementsRoutes(controller) } },
    ) {
        coEvery { entitlementsService.getEntitlements("signal", "org_1") } returns
    ...
    @Test
    fun `GET entitlements InternalError returns HTTP 500`() = billingTestApplication(
        routes = { noAuthInternalRoutes { entitlementsRoutes(controller) } },
    ) {
        coEvery { entitlementsService.getEntitlements("signal", "org_1") } returns
     ...
     */
    @Test
    fun `Property 5 - Found response preserves all fields for arbitrary planId`() = runBlocking {
        val nonBlankPlanId = Arb.string(minSize = 1, maxSize = 50).filter { it.isNotBlank() }

        forAll(nonBlankPlanId) { planId ->
            var result = false
            val stubResponse = EntitlementsResponse(
                appSlug = "signal",
                orgId = "org_1",
                planId = planId,
